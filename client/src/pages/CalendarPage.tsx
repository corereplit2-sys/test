...
import { SafeUser, Booking, BookingWithUser, CapacityInfo, BookableWeekRange } from "@shared/schema";
...
export default function CalendarPage() {
  ...
  return (
    <div className="min-h-screen bg-background">
          ...
          <div className="border rounded-md p-4 bg-card" data-testid="calendar-container">
            {weekLoading ? (
              ...
              <FullCalendar
                ...
                weekends={true}
                events={events}
                dateClick={user.role === "admin" || user.role === "commander" ? undefined : handleDateClick}
      ...
      <Dialog open={!!bookingModal && user.role !== "admin" && user.role !== "commander"} onOpenChange={() => setBookingModal(null)}>
        <DialogContent>
          <DialogHeader>
          ...
          {bookingModal && (
            <div className="space-y-4 py-4">
              ...
              {!hasEnoughCredits && (
                <div className="border border-destructive/50 rounded-md p-3 bg-destructive/5">
                  <p className="text-sm text-destructive font-medium">
                    Insufficient credits. You need {creditsNeeded} but only have {user.credits.toFixed(1)}.
                  </p>
                </div>
              )}
            </div>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={isCreating || !hasEnoughCredits || capacityInfo?.isFull || loadingCapacity || userAlreadyBooked}
              data-testid="button-confirm-booking"
            >
              {isCreating ? "Creating..." : userAlreadyBooked ? "Already Booked" : capacityInfo?.isFull ? "Slot Full" : "Confirm Booking"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
          ...
          {eventModal && (
            <div className="space-y-4 py-4">
              ...
              <div>
                ...
                <p className="text-lg font-mono font-semibold">{eventModal.creditsCharged.toFixed(1)}</p>
              </div>
