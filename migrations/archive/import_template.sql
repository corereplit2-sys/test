...
-- Step 2: Insert your Excel data here (ADD YOUR DATA BELOW)
-- Format: ('Activity', 'YYYY-MM-DD', 'Name', age, 'Result', total, situp_reps, situp_score, pushup_reps, pushup_score, 'run_time', run_score),
...
-- Step 3: Create sessions from unique IPPT dates
INSERT INTO ippt_sessions (
    id, name, date, totalAttendees, avgScore, goldCount, silverCount, passCount, failCount
)
SELECT 
    'session-' || MD5(ippt_date::TEXT) as id,
    'IPPT Test - ' || TO_CHAR(ippt_date, 'YYYY-MM-DD') as name,
    ippt_date as date,
    COUNT(*) as totalAttendees,
    ROUND(AVG(total_score), 2) as avgScore,
    COUNT(CASE WHEN result = 'Gold' THEN 1 END) as goldCount,
    COUNT(CASE WHEN result = 'Silver' THEN 1 END) as silverCount,
...
SELECT 
    ...
    'session-' || MD5(t.ippt_date::TEXT) as session_id,
    t.activity,
    t.ippt_date,
    t.age_as_of_ippt,
    t.situp_reps,
...
-- Step 8: Clean up (uncomment when done)
