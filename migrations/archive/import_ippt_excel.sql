...
-- Replace this with your actual Excel data
...
('MS Coy IPPT 1', '2025-08-20', 'MOHAMAD IYSHRAQ JADULHAQ BIN MOHAMMAD YUSMAN', 22, 'Pass', 67, 32, 15, 36, 19, '12:06', 33),
...
-- Add more rows from your Excel file here...

-- Note: Your CSV format: Activity, Date, Platoon, Rank, Name, DOB, Age, Result, Total, Situp Reps, Situp Score, Pushup Reps, Pushup Score, Run Time, Run Time Formatted, Run Score, Empty, Empty, Empty
-- We import columns: 1, 2, 5, 7, 8, 9, 10, 11, 12, 13, 16
-- We ignore columns: 3(Platoon), 4(Rank), 6(DOB), 14(Run Time), 15(Formatted Time), 17-19(Empty)
...
SELECT 
    'session-' || MD5(ippt_date::TEXT || activity::TEXT) as id,
    activity as name,
    ippt_date as date
...
LEFT JOIN users u ON u.full_name ILIKE t.name  -- Case-insensitive name matching
WHERE u.id IS NOT NULL  -- Only import matched users;
