...
export function AdminCurrencyDrives() {
  ...
  const [selectedDrive, setSelectedDrive] = useState<CurrencyDrive | null>(null);
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>{drives.length} active QR code{drives.length !== 1 ? 's' : ''}</CardDescription>
        </CardHeader>
        <CardContent>
          {drives.length === 0 ? (
              ...
              {drives.map((drive) => (
                <div key={drive.id} className="border rounded-md p-4 hover:bg-accent">
                  <div className="flex items-start justify-between gap-2 mb-3">
                    ...
                    <Badge variant="outline">{drive.code}</Badge>
                  </div>
                  <p className="text-xs text-muted-foreground mb-3">
                    Expires: {format(new Date(drive.expiresAt), "MMM d, yyyy")}
                  </p>
